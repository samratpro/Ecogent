...
\begin{figure}[H]
    \centering
    \vspace{-3cm}
    \includegraphics[width=0.7\textwidth]{images/ecogent_workflow.png}
...
Instead of saving long, full chat conversations—which wastes tokens and drives up costs—the system creates short, meaningful tags for each task. 
...
\end{itemize}
These simple tags are saved in a local database called Chroma DB. By using this database to search for similar tags, the system can instantly find past solutions, old workflows, or specific tools. This prevents the AI from having to read through massive conversation histories every time.\\
...
\begin{itemize}
    ...
    \item \textbf{Memory Bloat in Local Storage:} Over time, saving every workflow pattern could cause the local ChromaDB to become bloated, slowing down search times. To prevent this, a background caching policy will automatically prune old, unused, or obsolete workflows, ensuring the database remains lightweight and fast.
\end{itemize}
